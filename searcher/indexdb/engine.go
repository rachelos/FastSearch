...
type Engine struct {
	...
	InvertedIndexNStorages []*storage.LeveldbStorage //关键字和Id映射，倒排索引,key=id,value=[]words
...
func (e *Engine) Init() {
	...
	for shard := 0; shard < e.Shard; shard++ {
		...
		//初始化Keys存储
		ks, kerr := storage.NewStorage(e.GetFilePath(fmt.Sprintf("%s_%d", e.Option.InvertedIndexName, shard)), true, e.Timeout)
		if kerr != nil {
			panic(err)
		}
